...
async function ResetPedComponents() {
	...
	SetPedComponentVariation(ped, 8, 0, 0, 0);  // Undershirt rien
	SetPedComponentVariation(ped, 4, 11, 0, 0); // Pantalon
	SetPedComponentVariation(ped, 11, 0, 0, 0); // Top rien
...
RegisterCommand('screenshotdecal', async (source, args) => {
    // On utilise le modèle homme par défaut
    ...
    for (let drawable = 0; drawable < drawableVariationCount && !stop; drawable++) {
        ...
        while (!taken && !stop) {
            ...
            if (IsControlJustPressed(0, 191)) { // Entrée
                emitNet('takeScreenshot', `decal_${drawable}_${texture}`, 'decal');
